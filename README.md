...
이 저장소는 완전한 운영 서비스보다는, 다음 두 축을 함께 보여주는 포트폴리오 성격이 강합니다.

- 문서 자동화 코어: JSON -> HTML -> PDF
- 한국어 이슈 분석 실험: 수집 -> 전처리 -> 중복 제거 -> 토픽/NER/요약

즉, "분석 결과를 실제 문서 산출물까지 연결하는 자동화 프로젝트"로 보는 것이 가장 정확합니다.

## 아쉬운 점과 다음 개선 방향

- 의존성 관리 파일과 실행 엔트리포인트를 더 명확하게 정리할 필요가 있습니다.
- `test/` 디렉토리는 현재 자동화 테스트보다는 실험 스크립트 저장소에 가깝습니다.
- 수집, 요약, 렌더링 단계를 하나의 CLI 또는 배치 파이프라인으로 통합하면 재현성이 더 좋아집니다.
- 샘플 데이터 외 실제 입력 소스와 배치 실행 설정을 정리하면 운영형 프로젝트에 더 가까워질 수 있습니다.
